...
const articleName = document.querySelector('.home-header')
  ? '.home-header'
  : document.querySelector('.blog-listing')
  ? '.blog-listing'
  : null;
const mostViewsPosts = document.querySelector('.most-views-posts');
const searchLimitTags = document.querySelector('.search-tags');

getFetch('/backend/api/header_link/getAllHeaderLinks.php', getAllHeaderLinks);
getFetch(
  '/backend/api/quick_link/getAllQuickLinks.php',
  getFooterAllQuickLinks
);
  ...
  data
    ...
    .map((item) => {
      ...
      listItem.innerHTML = `
        ...
        item.target
      }' class='header__item-link navigation__item-link'>${
  ...
  data
    ...
    .map((item) => {
      ...
      listItem.innerHTML = `
            <a href='${item.url}' target='${
        item.target
      }' class='footer__item-link quick-link__item-link'>${
        item.title.slice(0, 1).toUpperCase() + item.title.slice(1)
      }</a>
        `;
...
function getFooterAllCategories(data) {
  data
    ...
    .map((item) => {
      ...
      listItem.innerHTML = `
            <a href='/category/index/${item.id}' target='${
        item.target
      }' class='footer__item-link category__item-link'>${
        item.title.slice(0, 1).toUpperCase() + item.title.slice(1)
      }</a>
        `;
...
async function setArticle() {
  ...
  if (getArticle && getAuthor && getCategory) {
    ...
    background.style.backgroundImage =
      "url('/frontend/images/articles/" + getArticle.image + ".webp')";
    ...
    authorImage.src = getAuthor.image
      ? getAuthor.image
      : '/frontend/images/no-picture/no-picture-36x36.png';
...
async function getArticleByUri(data) {
  ...
  const response = await fetch(
    '/backend/api/article/articleByUri.php',
    headers
  );
...
async function getLatestPost(count) {
  try {
    ...
    const response = await fetch(
      '/backend/api/post/getLatestPost.php',
      headers
    );
...
async function getAdminById(id) {
  try {
    ...
    const response = await fetch(
      '/backend/api/admin/getAdminById.php',
      headers
    );
...
async function getCategoryById(id) {
  try {
    ...
    const response = await fetch(
      '/backend/api/category/getCategoryById.php',
      headers
    );
...
async function getMostViewsPosts(count) {
  try {
    ...
    const response = await fetch(
      '/backend/api/post/getMostViewsPosts.php',
      headers
    );
    if (!response.ok) {
...
async function setMostViewPosts() {
  ...
  const arrayMostViewPosts = [
    {
      ...
      view: document.querySelector(
        '.most-views-posts__left-side > .most-views-posts__post'
      ),
    ...
    },
    {
      post: getPosts[1],
      view: document.querySelector(
        '.most-views-posts__right-side > .most-views-posts__post:first-child'
      ),
    ...
    },
    {
      post: getPosts[2],
      view: document.querySelector(
        '.most-views-posts__right-side > .most-views-posts__post:last-child'
      ),
  ...
  arrayMostViewPosts.map((item) => {
    ...
    const getDatePost =
      datePost.toLocaleString('default', { month: 'long' }) +
      ' ' +
      datePost.getDate() +
      ', ' +
      datePost.getFullYear();
    item.view.children[0].children[0].href =
      '/category/index/' + item.post.category_id;
    ...
    item.view.children[2].children[0].children[0].src = item.author.image
      ? '/frontend/images/authors/' + item.author.image + '.webp'
      : '/frontend/images/no-picture/no-picture-36x36.png';
    item.view.children[2].children[0].children[1].href =
      '/author/index/' + item.author.id;
    item.view.children[2].children[0].children[1].innerText =
  ...
  try {
    ...
    for (const item of posts) {
      ...
      listItem.innerHTML = `
                <img src="${
                  item.image
                    ? '/frontend/images/posts/' + item.image + '.webp'
                    : '/frontend/images/no-picture/no-picture-360x240.png'
                }" alt="no-picture post icon" class="latest-post__item-image">

                <div class="latest-post__item-category latest-post__category">
                    <a href="/category/index/${
                      item.category_id
                    }" class="latest-post__item-link latest-post__category-link">${
        category.title
      }</a>
                </div>

                <div class="latest-post__item-name latest-post__name">
                    <a href="/singlepost/index/${
                      item.id
                    }" class="latest-post__item-name latest-post__name-link">${
        item.title
      }</a>
                </div>
                ...
                <div class="latest-post__item-data">
                    <div class="latest-post__item-author latest-post__author">
                        <img src="${
                          admin && admin.image
                            ? '/frontend/images/authors/' +
                              admin.image +
                              '.webp'
                            : '/frontend/images/no-picture/no-picture-36x36.png'
                        }" alt="no-picture author" class="latest-post__author-icon">
                        <a href="/author/index/${
                          item.created_by
                        }" class="latest-post__author-name">${
...
async function getLimitTags(count) {
  try {
    const headers = {
      method: 'POST',
      headers: {
    ...
    const response = await fetch(
      '/backend/api/tags/getLimitedTags.php',
      headers
    );
    return await response.json();
  } catch (error) {
...
async function setLimitTags() {
  const listSearchTags = document.querySelector('.search-tags__list');
  const tags = await getLimitTags(8);

  tags.map((item) => {
    const li = document.createElement('li');
    li.classList.add('search-tags__item');
    li.innerHTML = `
        <a href='/tag/index/${item.id}' class='search-tags__item-href'>#${item.title}</a>
    `;
    listSearchTags.append(li);
  });
}
...
async function getTag(id) {
  try {
    ...
    const response = await fetch('/backend/api/tags/getTagById.php', headers);
    if (!response.ok) {
...
async function getSinglePostById(id) {
  try {
    ...
    const response = await fetch('/backend/api/post/getPostById.php', headers);
    if (!response.ok) {
...
async function setSinglePostPath() {
  const currentPath = window.location.pathname.split('/');
  let category, tag, author, singlePost;
  ...
  switch (currentPath[1]) {
    case '':
      ...
      break;
    case 'category':
      category = (await getCategoryById(currentPath[3])).title;
      sessionStorage.setItem('category', category);
      // Сделать объект с названием и id для ссылки
      break;
    case 'author':
      author = (await getAdminById(currentPath[3])).username;
      sessionStorage.setItem('author', author);
      break;
    case 'tag':
      tag = (await getTag(currentPath[3])).title;
      sessionStorage.setItem('tag', tag);
      break;
    case 'singlepost':
      post = (await getSinglePostById(currentPath[3])).title;
